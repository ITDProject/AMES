...
//maybe this is related with input output method
    ...
    public static String getStrings(double[][] a, int index, DecimalFormat DF) {
        //String[][] output = new String[a.length][];
        //int i = 0;
        String output = "";
        for (int i = 0; i < a.length; i++) {
            String temp1 = DF.format(a[i][index]);
            output = output + temp1;
    ...
    public void buildSchedule() {
        ...
        RTLMP = new double[getNumHoursPerDay()][getNumNodes()];
            ...
            public void execute() {
                ...
                if (FNCSActive) {
                    if (day == 1 && min % 60 ==0) {
                        ...
                        System.out.println("time granted 1: " + time_granted);
                    } 
                    if (day > 1 && (min % M ==0))   {
                        time_next = (day - 1) * (day_len) + (hour - 1) * (hour_len) + min * (min_len) + count;
                            //time_next = (day - 1) * (day_len) + (hour - 1) * (hour_len) + min * (min_len) - M * min_len + 1;
                        System.out.print("time requested 1 : " + time_next);
                        while (time_granted < time_next) {
                            time_granted = JNIfncs.time_request(time_next);
                            System.out.print(" time granted 1: " + time_granted);
                        }
                        System.out.println(" Final time granted 1 : " + time_granted);
                    }
                }

                iso.marketOperation(min, interval, hour, day, FNCSActive, count);
                ...
                if (!SolutionStatus.equals("infeasible")) {
                    ...
                    if ((day== 1 && hour==13 && min==0)|| (hour == 13 && min == 0 && count==5)) {
                        System.out.println("");
                        for (int n = 0; n < getNumNodes(); n++) {
                            JNIfncs.publish("DALMPAtBus"+ String.valueOf(n+1), getStrings(iso.getDAMLMP(), n, LMPFormat));
                            System.out.println("Publishing DAM LMP values of Bus" + (n + 1) + " : " + getStrings(iso.getDAMLMP(), n, LMPFormat));
                        }
                        System.out.println("");
                    }
                    ...
                    if (FNCSActive) {
                        if (day > 1 && min % M == 0 && count==5) {
                            time_next = (day - 1) * (day_len) + (hour - 1) * (hour_len) + min * (min_len) + 1 * min_len;
                                //time_next = (day - 1) * (day_len) + (hour - 1) * (hour_len) + min * (min_len) - M * min_len + 1 * min_len;
                            System.out.print("time requested 2 : " + time_next);
                            time_granted = JNIfncs.time_request(time_next);
                            while (time_granted < time_next) {
                                time_granted = JNIfncs.time_request(time_next);
                                System.out.print(" time granted 2: " + time_granted);
                            }
                            System.out.println(" Final time granted 2 : " + time_granted);
                        }
                    }
                    ...
                    if (day > 1 && min % M == 0 && count==5) {
                        ...
                        for (int i = 0; i < getNumNodes(); i++) {
                            ...
                            AvgLMP = sum / NIRTM;
                            System.out.println(": " + LMPFormat.format(AvgLMP));
                            RTLMP[hour - 1][i] = AvgLMP;
                            JNIfncs.publish("RTLMPAtBus"+ String.valueOf(i+1), LMPFormat.format(AvgLMP));
                        }
                        System.out.println("");
                    }
                    ...
                    if (FNCSActive) {
                        if (day == 1 && min % M ==0) {
                            time_next = (day - 1) * (day_len) + (hour - 1) * (hour_len) + (min + M) * min_len;
                            System.out.print("time requested 3 : " + time_next);
                            time_granted = JNIfncs.time_request(time_next);
                            while (time_granted < time_next) {
                                time_granted = JNIfncs.time_request(time_next);
                                System.out.print("  time granted 3: " + time_granted);
                            }
                            System.out.println(" Final time granted 3 : " + time_granted);
                        }
                        if (day > 1 && min % M == 0 && count==5) {
                            time_next = (day - 1) * (day_len) + (hour - 1) * (hour_len) + (min + M) * min_len;
                            System.out.print("time requested 3 : " + time_next);
                    ...
                    if (bMaximumDay) {
                        ...
                        if ((hour == 24) && (day == dayMax) && ((min+M) % min_len == 0) && (count == 5)) {// Only dayMax has been reached

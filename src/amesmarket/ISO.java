...
public class ISO {
    ...
    private double[][] realtimeNDG; 
    
    private double[][] sumrealtimeload;
    private double[][] sumrealtimeNDG;
    ...
    public ISO(AMESMarket model) {
        ...
        M = ames.M;
        supplyOfferRT = new double[I][GEN_OFFER_PARAM_LEN];
    ...
    public void marketOperation(int m, int interval, int h, int day, boolean FNCSActive, int count) {

        
        if (day > 1 && m % this.ames.M == 0 && count==1) {
            rtm.RTMCollectBidsAndOffers(h, day-1, FNCSActive);
            
            this.RTPSLdemandBidByLSE = this.rtm.getRTPSLDemandBidByLSE();
            sumrealtimeload = new double[J][(int) this.ames.NIRTM];
            sumrealtimeNDG = new double[J][(int) this.ames.NIRTM];
        }   
        
        if (day > 1 && (m % M ==0)) {
            System.out.println("Waiting to receive bids/offers for RTM operation at count: "+ count);
            ...
            if(count==5){
                System.out.println("Bids/offers receiving window is closed.");
            }
        }
        ...
        if ((h == 24) && (day < this.ames.DAY_MAX) && m == 0 && count==1) {
    ...
    private double[][] getRealTimeLoadForecast(int h, boolean FNCSActive) {
        ...
        if (FNCSActive) {
            ...
            for (int i = 0; i < events.length; ++i) {
                ...
                for (int j = 0; j < J; j++) {
                    if (events[i].equals("loadforecastRTM_LSE" + String.valueOf(j + 1))) {
                        System.out.println("Received RTM loadforecast: " + values[0]);
    ...
    public void endOfDayCleanup() {
        ...
        for (int i = 0; i < this.I; i++) {
            for (int j = 0; j < this.supplyOfferByGen[0].length; j++) {
